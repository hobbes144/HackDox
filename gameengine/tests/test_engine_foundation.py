...
def test_every_domain_the_generator_produces_is_classified():
    ...
    consumer = {"gmail.com", "portmail.net", "fastmail.io", "fogbank.me"}
...
_DW01_IDENTITY_LENIENCY = {
    ...
    "justification": (
        ...
        " plenty of people on this waterfront run alt accounts for "
        "perfectly boring reasons, and turning them back at the gate makes "
        "us look paranoid. Starting today it's advisory only: flag the "
        "pattern, use your judgement, don't auto-deny on it alone. Minor "
        "tightening of scope, nothing more."),
...
_DW02_FORUM_LENIENCY = {
    ...
    "justification": (
        ...
        "scraped, names get reused. We don't deny a person for which docks "
        "their name washes up on. It's a flag from this morning, not a "
        "deny. I know how that sounds. Note it and move on."),
...
_DW03_STUFFING_LENIENCY = {
    ...
    "justification": (
        ...
        " a legitimate surge from an attack anymore — we nearly turned back"
        " half a real crew's rollout last week because the pattern looked "
        "the same on paper. Credential stuffing is advisory only until "
        "someone actually fixes the false-positive rate. Flag it. Don't "
        "kill it on sight."),
...
_DW04_PAYLOAD_LENIENCY = {
    ...
    "justification": (
        ...
        "sealed crate doesn't prove what's in it, and I'm done pretending "
        "it does. An encrypted payload is a flag now, not a deny — and if "
        "you've got a problem with that, take it up with whoever actually "
        "reads these policy memos, because it isn't me."),
...
_DW05_PAYLOAD_CRACKDOWN = {
    ...
    "mutability": "dark_web",
    "justification": (
        "Forget what I told you about encryption not being a crime. Whoever"
        " we almost let off the boat last week knew exactly how to hide "
...
_DW06_HOSTILE_PAYLOAD_LENIENCY = {
    ...
    "justification": (
        ...
        "with the parallel strokes — the 'hostile' ones — upstairs says a "
        "few lines in a picture aren't a confession. Flag them, let them "
        "through. Yes, the same morning they battened down encryption. "
        "Don't ask me to make that add up; I've stopped trying."),
...
def test_simulate_command_does_not_crash_on_the_banded_only_days():
    ...
    for day_n in (14, 15, 16, 18, 19):
        ...
        assert "Foreman" in result.output, (
            f"day {day_n}: simulate produced no Foreman panel at all")
